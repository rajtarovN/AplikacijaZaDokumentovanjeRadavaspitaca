...
@Component({
  selector: 'jhi-vaspitac',
  templateUrl: './vaspitac.component.html',
})
export class VaspitacComponent implements OnInit {
  vaspitacs?: IVaspitac[];
  isLoading = false;
  totalItems = 0;
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.vaspitacService
      ...
      .subscribe({
        next: (res: HttpResponse<IVaspitac[]>) => {
  ...
  trackId(_index: number, item: IVaspitac): number {
    return item.id!;
  }

  delete(vaspitac: IVaspitac): void {
    const modalRef = this.modalService.open(VaspitacDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.vaspitac = vaspitac;

...
export class FormularComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.username) {
      ...
      this.formularService
        ...
        .subscribe({
          next: (res: HttpResponse<IFormular[]>) => {
            this.isLoading = false;
            this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate);
          },
          error: () => {
  ...
  trackId(_index: number, item: IFormular): number {
    return item.id!;
  }

  delete(formular: IFormular): void {
    const modalRef = this.modalService.open(FormularDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.formular = formular;
  ...
  protected onSuccess(data: IFormular[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
